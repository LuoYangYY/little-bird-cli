...
let  init = async () => {
    ...
    try {
        ...
        await loadCmd(`curl -u "LuoYangYY:34f880e58434244e3dfca70ef646eeb27ad0b0e9" https://api.github.com/user/repos -d '{"name": "${projectName}"}'`, 'Github仓库创建')
        await loadCmd(`git remote add origin https://github.com/LuoYangYY/${projectName}.git`, '关联远端仓库')
        let loading = ora();
        loading.start(`package.json更新repository: 命令执行中...`);
        await updateJsonFile('package.json', {
            "repository": {
                "type": "git",
                "url": `https://github.com/LuoYangYY/${projectName}.git`
            }
        }).then(() => {
